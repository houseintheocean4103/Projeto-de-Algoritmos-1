#include <iostream>           //qualquer linguagem ordena as letras alfabeticamente, dessa forma é possível
using namespace std;          //utilizar maior, menor, igual para ordenar strings alfabeticamente
...
void selection_sort(pais vetor[], int tam){
    ...
    for (indice = 0; indice < tam-1; indice++) {
        ...
        for (j = indice + 1; j < tam; j++) {
            if (vetor[j].nome < vetor[menor].nome){
                menor = j;
            }
        }
        aux = vetor[indice];
